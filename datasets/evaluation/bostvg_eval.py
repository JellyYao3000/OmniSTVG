...
import json
from utils.matcher import HungarianMatcher2
...
class BOSTVGiouEvaluator:
    def __init__(
        ...
        """
        :param bostvg_path: path to BOSTVG annotations
        :param subset: train, val or test
        :param iou_thresholds: IoU thresholds for the vIoU metrics
        """
        assert subset in ["train", "test"], f"Wrong BOSTVG subset {subset}"
    
        gt_data = []
        cache_dir = os.path.join(bostvg_path, 'data_cache')
        dataset_cache = os.path.join(cache_dir, f'bostvg-{subset}-anno.cache')
        ...
        for data_item in gt_data:
            ...
            new_box_dict = {}
            for b in box_dict: 
                for fid,value in b.items():
                    if fid in new_box_dict.keys():
                        new_box_dict[fid].append(value)
                    else:
                        new_box_dict[fid] = [value]
            self.vid2box[video_id]=new_box_dict
        ...
        for video_id, video_pred in video_predictions.items():
            ...
            for fid in self.vid2box[video_id].keys():  # iterate on all frames of the annotated moment to update GT metrics
                if fid not in prediction:
                    ...
                    continue
                pred_boxes = np.array(prediction[fid])[0]
                gt_boxes = np.array(self.vid2box[video_id][fid])
                try:
                    index = matcher(torch.Tensor(pred_boxes).unsqueeze(0), torch.Tensor(gt_boxes).unsqueeze(0))[0]
                    pred_boxes = pred_boxes[index[0]].reshape(-1, 4)
                    gt_boxes = gt_boxes[index[1]].reshape(-1, 4)
                    iou = np.mean(np.diag(np_box_iou(pred_boxes, gt_boxes)))
                except:
                    print(pred_boxes.shape, gt_boxes.shape)
                    iou = 0
...
class BOSTVGEvaluator(object):
    def __init__(
        ...
        """
        self.evaluator = BOSTVGiouEvaluator(
            bostvg_path,
            subset=subset,
