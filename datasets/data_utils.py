...
def crop_for_2d_map(cfg, video_data):
    ...
    if cfg.DATASET.NAME == 'BOSTVG':
        ...
        raise ValueError("{} is not Supported".format(cfg.DATASET.NAME))
...
def make_bostvg_input_clip(cfg, split, video_data):
    ...
    video_fps = frame_count / video_data['duration']
    ...
    if sum(gt_mask[sample_slice]) < 2:
        video_frames[-2] = list(gt_mask).index(1)
        video_frames[-1] = min(list(gt_mask).index(1) + 1, len(gt_mask)-1)
        sample_slice[-2] = list(gt_mask).index(1)
        sample_slice[-1] = min(list(gt_mask).index(1) + 1, len(gt_mask)-1)
        sample_slice.sort()
        video_frames.sort()
