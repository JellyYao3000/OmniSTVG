    ...
    def __call__(self, input_dict):
        if random.random() < self.prob:
            ...
            for i in range(len(boxs)):
                boxs[i]['bbox'] = boxs[i]['bbox'].transpose(0)
...
class RandomResize(object):
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(boxs)):
            boxs[i]['bbox'] = boxs[i]['bbox'].resize((size[1],size[0]))
        input_dict['frames'] = frames
        input_dict['boxs'] = boxs
...
class Normalize(object):
    ...
    def __call__(self, input_dict):
        ...
        assert  boxs[0]['bbox'].size == (frames.shape[3],frames.shape[2])  # (w, h)
        for i in range(len(boxs)):
            boxs[i]['bbox'] = boxs[i]['bbox'].normalize()
        input_dict['frames'] = frames
        input_dict['boxs'] = boxs
        return input_dict
